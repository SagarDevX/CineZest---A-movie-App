...
const Documentry = () => {
    ...
    const DataFetch = async () => {
      try {
        ...
        const [page1] = await Promise.all([
          fetch(`${BASE_URL}?endpoint=discover/tv&with_genres=99&sort_by=popularity.desc&page=2`).then(r => r.json()),
        ])
  ...
  return (
    <div className="h-full max-w-8xl mx-auto mt-24  text-white flex-wrap items-center justify-center px-4 md:px-12">
      ...
      <p>Documentaries that go beyond the surface — exploring reality, perspectives, and the stories that shape our world.</p>
        </div>
