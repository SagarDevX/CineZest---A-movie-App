...
const Action = () => {
    ...
    const DataFetch = async () => {
      try {
        ...
        const [page1, page2, page3] = await Promise.all([
          fetch(`${BASE_URL}?endpoint=discover/tv&with_genres=10759&sort_by=popularity.desc&page=1`).then(r => r.json()),
          fetch(`${BASE_URL}?endpoint=discover/tv&with_genres=10759&sort_by=popularity.desc&page=2`).then(r => r.json()),
          fetch(`${BASE_URL}?endpoint=discover/tv&with_genres=10759&sort_by=popularity.desc&page=3`).then(r => r.json()),
        ])
  ...
  return (
    <div className="h-full max-w-8xl mx-auto mt-24  text-white flex-wrap items-center justify-center px-4 md:px-12">
      ...
      <p>Action shows that keep you on the edge — where every second counts and every move matters.</p>
        </div>
